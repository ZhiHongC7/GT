#-------------------------------------------------------------
# Generated Example Tcl script for IP 'gt_app' (xilinx.com:ip:gtwizard:3.6)
#-------------------------------------------------------------
...
set srcIpDir "c:/Users/Administrator/Desktop/GT/gt_app/gt_app.srcs/sources_1/ip/gt_app"

# Create project
puts "INFO: \[open_example_project\] Creating new example project..."
create_project -name gt_app_ex -force
...
import_ip -files [list [file join $srcIpDir gt_app.xci]] -name gt_app
reset_target {open_example} [get_ips gt_app]
...
generate_target -quiet [_filter_supported_targets {instantiation_template synthesis simulation implementation shared_logic} [get_ips gt_app]] [get_ips gt_app]
...
add_files -scan_for_includes -quiet -fileset [current_fileset] \
  [list [file join $srcIpDir gt_app/example_design/support/gt_app_common_reset.v]] \
  [list [file join $srcIpDir gt_app/example_design/support/gt_app_clock_module.v]] \
  [list [file join $srcIpDir gt_app/example_design/support/gt_app_common.v]] \
  [list [file join $srcIpDir gt_app/example_design/support/gt_app_gt_usrclk_source.v]] \
  [list [file join $srcIpDir gt_app/example_design/support/gt_app_support.v]] \
  [list [file join $srcIpDir gt_app/example_design/gt_app_exdes.v]] \
  [list [file join $srcIpDir gt_app/example_design/gt_app_gt_frame_check.v]] \
  [list [file join $srcIpDir gt_app/example_design/gt_app_gt_frame_gen.v]]
...
add_files -quiet -fileset [current_fileset] \
  [list [file join $srcIpDir gt_app/example_design/gt_rom_init_tx.dat]]
set_property USED_IN_SIMULATION false [get_files [list [file join $srcIpDir gt_app/example_design/gt_rom_init_tx.dat]]]
...
add_files -quiet -fileset [current_fileset -constrset] \
  [list [file join $srcIpDir gt_app/example_design/gt_app_exdes.xdc]]
...
add_files -quiet -scan_for_includes -fileset [current_fileset -simset] \
  [list [file join $srcIpDir gt_app/simulation/gt_app_tb.v]]
set_property USED_IN_SYNTHESIS false [get_files [list [file join $srcIpDir gt_app/simulation/gt_app_tb.v]]]
...
add_files -quiet -fileset [current_fileset -simset] \
  [list [file join $srcIpDir gt_app/simulation/functional/gt_rom_init_tx.dat]]
set_property USED_IN_SYNTHESIS false [get_files [list [file join $srcIpDir gt_app/simulation/functional/gt_rom_init_tx.dat]]]
...
if {[catch {source [file join $srcIpDir gt_app/tcl/xci.tcl]} errMsg]} {
  puts "ERROR: \[open_example_project\] Open Example Project failed: Error encountered while sourcing custom IP example design script."
  puts "$errorInfo"
  error "ERROR: see log file for details."
  incr returnCode
}
if {[catch {source [file join $srcIpDir gt_app/tcl/gt_app_partner.tcl]} errMsg]} {
  puts "ERROR: \[open_example_project\] Open Example Project failed: Error encountered while sourcing custom IP example design script."
  puts "$errorInfo"
  error "ERROR: see log file for details."
  incr returnCode
